...
class WinHooks {
  final methodChannel = const MethodChannel('win32hooks');
  ...
  Future<bool> uninstallWinHook() async {
    hookRunning = false;
    final result = await methodChannel.invokeMethod('uninstallHooks');
    print(result);
    return true;
  }
  ...
  Future<bool> uninstallSpecificHookID(int hookID, HookTypes hookType) async {
    final result = await methodChannel.invokeMethod('uninstallSpecificHookID', {'hookID': hookID, 'hookType': hookType.index});
    print(result);
    return true;
  }
}
